...
class Trainer(object):
    def __init__(self, model, n_gpu, logger, optimizer, lr_scheduler,
        ...
        self.device, _ = tools.prepare_device(n_gpu)
        self.id2label = {id:label for label, id in self.label2id.items()}
        self.entity_score = ner_utils.SeqEntityScore(self.id2label)
        self.start_epoch = 1
        self.global_step = 0

        if resume_path:
            self.logger.info(f'\nLoadingi checkpoint: {resume_path}')
    ...
    def valid_epoch(self, data_loader):
        pbar = progressbar.ProgressBar(n_total=len(data_loader), desc='Evaluating')
    ...
    def train_epoch(self, data_loader):
        pbar = progressbar.ProgressBar(n_total=len(data_loader), desc='Training')
    ...
    def train(self, train_data, valid_data, epochs, seed):
        ...
        for epoch in range(self.start_epoch, self.start_epoch + int(epochs)):
            ...
            # save model
            if self.model_checkpoint:
                state = self.save_info(epoch, best = logs[self.model_checkpoint.monitor]) # monitor是‘valid_f1’,用该指标监控
