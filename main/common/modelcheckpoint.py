...
class ModelCheckpoint(object):
    def __init__(self, checkpoint_dir, monitor, arch, mode='min', 
                epoch_freq = 1, best = None, save_best_only = True):
    ...
    def epoch_step(self, state, current):
        if self.save_best_only:
            ...
            file_name = self.base_path / f"EPOCH_{state['epoch']}_{state[self.monitor]}_{self.arch}_MODEL.pth"
            if state['epoch'] % self.epoch_freq == 0:
                tools.logger.info(f"\nEpoch {state['epoch']}: save model to disk.")
                torch.save(state, str(file_name))

    def bert_epoch_step(self, state, current):
        # for bert type mode, for pytorch_transformer
        ...
        if self.save_best_only:
            if self.monitor_op(current, self.best):
                ...
                self.best = current
                state['best'] = self.best
                tools.logger.info(f'\nSaving current best model module | config file | checkpoint info to {self.base_path}')
